...
export default function Home({ user, handleSubmit, authorized_user, isLoaded }) {
  
  return (<>{isLoaded ? <Routes>
     <Route path='/incidents/*' element={<IncidentIndex 
            user={user} 
            ...
            authorized_user={authorized_user}
            />} />
             <Route exact path='/*' element={<IncidentIndex 
            user={user} 
            handleSubmit={handleSubmit} 
            authorized_user={authorized_user}
            />} />
  </Routes>: <p>Loading app</p> }</>)
}
